...
test("内置数据包：结构完整 + 关键映射正确", () => {
  ...
  assert.strictEqual(
    pack.gameNames[normZh("最后的生还者2：重制版")],
...
test("rememberAppId：本地积累写入缓存，下次查询离线命中", () => {
  const before = getActiveDataPackVersion();
  assert.strictEqual(rememberAppId("Titan Quest II", "1154030", { zhName: "泰坦之旅2" }), true);
  const pack = getActiveDataPack();
  assert.ok(pack.version > before, "缓存版本应递增");
  assert.strictEqual(pack.appIds[normEn("Titan Quest II")], "1154030");
  assert.strictEqual(pack.gameNames[normZh("泰坦之旅2")], "Titan Quest II");
  // 非法入参静默失败
  assert.strictEqual(rememberAppId("", "1154030"), false);
  assert.strictEqual(rememberAppId("X", "abc"), false);
});
