// lib/logger.js —— 按日滚动日志（镜像 biliup-hub/lib/logger.js，保持三端一致）
// 落 material-hub/logs/app-YYYY-MM-DD.log，stdout 同显；前端进度日志直接转发 child_process 行。
const fs = require('fs');
...
function cleanOldLogs() {
  try {
    ...
    if (removed) {
      try { console.log('[cleanup] 已清理 ' + removed + ' 个过期日志文件'); } catch (e) {}
    }
  } catch (e) {}
}
