...
test('getShareList：带提取码时调用 verify 并写入 BDCLND', async () => {
  ...
  global.fetch = makeFetch((url) => {
    if (url.includes('/api/gettemplatevariable')) return [bdstokenOk()];
    if (url.includes('/share/verify')) { verifyCalled = true; return [{ errno: 0, randsk: 'RAND_SK' }]; }
    if (url.includes('/s/')) return [SAMPLE_HTML, 200, { 'Content-Type': 'text/html' }];
    return [{}, 200, {}];
  });
...
test('getShareList：解析失败（无文件列表）抛错', async () => {
  ...
  global.fetch = makeFetch((url) => {
    if (url.includes('/api/gettemplatevariable')) return [bdstokenOk()];
    if (url.includes('/s/')) return ['{"shareid":1,"share_uk":"2"}', 200, { 'Content-Type': 'text/html' }];
    return [{}, 200, {}];
  });
...
test('transfer：errno 0 视为成功', async () => {
  ...
  global.fetch = makeFetch((url) => {
    if (url.includes('/api/gettemplatevariable')) return [bdstokenOk()];
    if (url.includes('/share/transfer')) return [{ errno: 0, task_id: 'T1', file_list: [{ fs_id: 111, path: '/x', server_filename: 'a', size: 1 }] }];
    return [{}, 200, {}];
  });
...
test('transfer：errno 4（目录已存在同名文件）也视作成功', async () => {
  ...
  global.fetch = makeFetch((url) => {
    if (url.includes('/api/gettemplatevariable')) return [bdstokenOk()];
    if (url.includes('/share/transfer')) return [{ errno: 4, task_id: 'T2', file_list: [], duplicated: { list: [{ fs_id: 111, path: '/x', server_filename: 'a', size: 1 }] } }];
    return [{}, 200, {}];
  });
  const r = await baidu.transfer('123', '765', ['111'], '/apps/netdisk_hub');
...
test('transfer：errno 12（文件已存在）视作成功', async () => {
  ...
  global.fetch = makeFetch((url) => {
    if (url.includes('/api/gettemplatevariable')) return [bdstokenOk()];
    if (url.includes('/share/transfer')) return [{ errno: 12, task_id: 'T3', file_list: [{ fs_id: 111, path: '/x', server_filename: 'a', size: 1 }] }];
    return [{}, 200, {}];
  });
  const r = await baidu.transfer('123', '765', ['111'], '/apps/netdisk_hub');
...
test('createShare：返回链接内嵌提取码 8888', async () => {
  ...
  global.fetch = makeFetch((url) => {
    if (url.includes('/api/gettemplatevariable')) return [bdstokenOk()];
    if (url.includes('/share/set')) return [{ errno: 0, link: 'https://pan.baidu.com/s/AbCdEfG9', shareid: 555 }];
    return [{}, 200, {}];
  });
