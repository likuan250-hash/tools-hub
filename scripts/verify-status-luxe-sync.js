...
//
// 校验 status-luxe 的「单一事实来源」shared/status-luxe 与五处前端副本
// （renderer / netdisk-hub/public / kdocs-tool/public / biliup-hub/public / xfer-hub/public）的 css 与 js
// 是否逐字节一致。任一副本与真源或彼此不一致则 exit(1) 并打印差异。
//
// 这样「真源」被纳入门禁：改了 shared/ 但忘了同步五副本 → CI 红灯；
...
for (const f of FILES) {
  ...
  if (mismatched.length) {
    ...
    console.log(`[ OK ] ${f} 真源与五处前端副本一致 (${base.hash})`);
  }
}

if (failed) {
  console.error('\nstatus-luxe 同步校验未通过：真源 shared/status-luxe 与五处前端副本必须逐字节一致。');
  console.error('如需以 shared/ 为准同步五副本，运行: npm run sync-status-luxe');
  process.exit(1);
}
console.log('\nstatus-luxe 真源与五处前端副本全部逐字节一致。');
