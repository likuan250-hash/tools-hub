...
if (params.get("authorized") === "baidu") {
  ...
  setBanner(
    "off",
    "尚未配置百度应用凭证,请在 .env 填写 BAIDU_CLIENT_ID / BAIDU_CLIENT_SECRET 后重启",
  );
...
async function loadAccounts() {
  ...
  cards.innerHTML = names
    .map(
      (n) =>
        `<div class="card"><div class="name">${n}</div><div class="status">${statusHTML("off", "检测中…")}</div></div>`,
    )
    .join("");
  try {
    ...
    cards.innerHTML = defs
      .map(({ key, name, a }) => {
        ...
        if (connected && a.expiresAt) {
          ...
          if (days <= 0) {
            ttlHTML =
              '<div><span class="ttl ttl-dead"><span class="p"></span>登录态已过期，请重新授权</span></div>';
          } else if (days <= 7) {
...
function renderTasks() {
  ...
  document
    .querySelectorAll("#historyTabs .tab")
    .forEach((b) => b.classList.toggle("active", b.getAttribute("data-filter") === historyFilter));
  ...
  box.innerHTML = groupArr
    .map(({ title, list }, gi) => {
      ...
      const sorted = [...list].sort(
        (a, b) => (PROVIDER_ORDER[a.provider] ?? 9) - (PROVIDER_ORDER[b.provider] ?? 9),
      );
      const rows = sorted
        .map((t) => {
          const name =
          ...
          const share = t.shareLink
            ...
            : t.status === "success"
              ? '<div class="meta">未生成分享</div>'
              : `<div class="meta" style="color:var(--err)">${escapeHtml(t.error || "")}</div>`;
...
function parseBatch(text) {
  ...
  const linkRe =
    /(pan\.baidu\.com\/s\/[A-Za-z0-9_-]+(?:\?[^\s#]*)?)|(pan\.xunlei\.com\/s\/[A-Za-z0-9_-]+(?:\?[^\s#]*)?)|(pan\.quark\.cn\/s\/[A-Za-z0-9_-]+(?:\?[^\s#]*)?)/;
  ...
  lines.forEach((line, i) => {
    ...
    if (lm) {
      ...
      // 同行提取码：迅雷常见「链接：... 提取码：xxxx」同在一行，链接行不能整个跳过
      const sameLinePwd = line.slice(lm.index + lm[0].length).match(pwdRe);
      jobs[provider] = { provider, link, pwd: pm ? pm[1] : sameLinePwd ? sameLinePwd[1] : "" };
      order.push(provider);
...
function getSelectedJobs() {
  const checked = Array.from(providerChecks.querySelectorAll('input[type="checkbox"]:checked')).map(
    (el) => el.value,
  );
...
function buildCopyAllText(results) {
  const ok = (results || []).filter(
    (r) => r.ok && r.share && isValidShareLink(r.share.link, r.provider),
  );
  if (!ok.length) return "";
...
function renderTransferStep(s) {
  ...
  item.innerHTML =
    ...
    icon +
    '</span><div class="step-body"><div class="step-name">' +
    statusHTML(lvl, escapeHtml(s.name + " — " + s.status)) +
    "</div>" +
  ...
  return new Promise((resolve) => {
    ...
    es.onmessage = (ev) => {
      ...
      if (d.type === "step" && d.step) renderTransferStep(d.step);
        ...
        transferAddLog("ok", "转存完成：成功 " + d.okCount + "/" + d.total);
        es.close();
        transferES = null;
      }
    };
...
batchBtn.onclick = async () => {
  ...
  try {
    ...
    if (!d.ok) {
      ...
      batchErr.innerHTML = ico("cross") + " " + escapeHtml(d.error);
      return;
    }
    ...
    batchErr.className = "err show";
    batchErr.innerHTML = statusHTML("err", ico("cross") + " " + e.message);
  } finally {
...
function buildCopyGroupText(group) {
  const PROVIDER_ORDER = { baidu: 0, quark: 1, xunlei: 2 };
  const ok = (group.list || []).filter(
    (t) => t.status === "success" && t.shareLink && isValidShareLink(t.shareLink, t.provider),
  );
  if (!ok.length) return "";
...
document.getElementById("historyIconBtn").addEventListener("click", () => {
  openModal(historyMaskEl);
});
document.getElementById("fmtIconBtn").addEventListener("click", () => {
  openModal(fmtMaskEl);
});
...
if (historyMaskEl)
  historyMaskEl.addEventListener("click", (e) => {
    if (e.target === historyMaskEl) closeModal();
  });
if (fmtMaskEl)
  fmtMaskEl.addEventListener("click", (e) => {
    if (e.target === fmtMaskEl) closeModal();
  });
...
async function openDirPicker(provider) {
  dirCtx = {
    provider,
    stack: [{ id: ROOT_ID[provider], name: "根目录" }],
    folders: [],
    loading: false,
  };
  ...
  document.getElementById("dirModalTitle").textContent =
    "选择「" + (PROVIDER_NAME[provider] || provider) + "」转存目录";
  renderBreadcrumb();
...
async function browseDir(parentId) {
  ...
  listEl.innerHTML =
    '<div class="empty-state"><span class="es-ico">' + ico("hourglass") + "</span>加载中…</div>";
  document.getElementById("dirHint").textContent = "";
  try {
    const r = await fetch(
      "/api/dirs/" + dirCtx.provider + "/browse?parent=" + encodeURIComponent(parentId),
    );
    const d = await r.json();
    if (!r.ok) {
      listEl.innerHTML =
        '<div class="empty-state" style="color:var(--err)"><span class="es-ico">' +
        ico("warning") +
        "</span>" +
        escapeHtml(d.error || "加载失败") +
        "</div>";
      return;
    }
...
function renderDirList() {
  ...
  listEl.innerHTML = folders
    .map(
      (
        f,
      ) => `<div class="dir-item" onclick="enterDir('${escapeHtml(f.id)}','${escapeHtml(f.name)}')">
...
function formatPost(text) {
  ...
  lines.forEach((line, i) => {
    for (const key of ["baidu", "xunlei", "quark"]) {
      ...
      // 同行提取码：迅雷常见「链接：... 提取码：xxxx」同在一行，链接行不能整个跳过
      const sameLinePwd = line.slice(m.index + m[0].length).match(pwdRe);
      jobs[key] = { link: full, pwd: pm ? pm[1] : sameLinePwd ? sameLinePwd[1] : "", prefix };
  ...
  lines.forEach((line, i) => {
    ...
    else if (bareCodeRe.test(line.trim()) && linkMeta.length)
      pwdCandidates.push({ lineNo: i, pwd: line.trim(), keyword: false });
  });
  ...
  const HEADER_RE =
    /^(游戏介绍|内容简介|简介|说明|本帖隐藏|百度\/|迅雷\/|夸克\/|提取码|密码|访问码|口令|分享|资源|下载)[:：]?/;
  let title = "";
  ...
  for (const key of ["baidu", "xunlei", "quark"]) {
    ...
    const link = j.link.includes("pwd=")
      ? j.link
      : j.pwd
        ? j.link + (j.link.includes("?") ? "&" : "?") + "pwd=" + j.pwd
        : j.link;
