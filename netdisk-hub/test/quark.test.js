...
test("parseLink：从链接提取 pwdId 与 passcode", () => {
  assert.deepStrictEqual(quark.parseLink("https://pan.quark.cn/s/AbC123"), {
    pwdId: "AbC123",
    passcode: "",
  });
  assert.deepStrictEqual(quark.parseLink("https://pan.quark.cn/s/XyZ?pwd=7788"), {
    pwdId: "XyZ",
    passcode: "7788",
  });
  ...
  global.fetch = makeFetch(() => [
    {
      code: 0,
      data: {
        list: [{ fid: "f1", share_fid_token: "t1", file_name: "a.iso", size: 100, dir: false }],
      },
    },
  ]);
...
test("saveShare：返回 task_id", async () => {
  ...
  const r = await quark.saveShare("cookie", {
    pwdId: "p",
    stoken: "s",
    fidList: ["f1"],
    fidTokenList: ["t1"],
...
test("createShare：异步（仅 task_id）轮询后按 fid_list 精确匹配我的分享", async () => {
      ...
      return [
        {
          ...
          data: {
            list: [
              ...
              {
                share_id: "SH1",
                share_url: "https://pan.quark.cn/s/Match01",
                fid_list: ["f1"],
                passcode: "p1",
              },
            ],
  ...
  assert.strictEqual(
    r.link,
    "https://pan.quark.cn/s/Match01",
    "应匹配到含 fid_list f1 的分享而非 list[0]",
  );
...
test("transfer：完整编排（无分享）", async () => {
  ...
  global.fetch = makeFetch((url, opts) => {
      ...
      return [
        {
          code: 0,
          data: {
            list: [{ fid: "f1", share_fid_token: "t1", file_name: "a.iso", size: 9, dir: false }],
          },
        },
      ];
    }
    ...
    if (url.includes("/task")) return [{ code: 0, data: { status: 2 } }];
    if (url.includes("/file?") && opts && opts.method === "POST") {
      ensured = true;
      return [{ code: 0, data: { fid: "DEST1" } }];
    }
    return [{}, 200, {}];
  });
...
test("transfer：生成分享时列出目标目录并按名匹配新文件", async () => {
  ...
  global.fetch = makeFetch((url, opts) => {
    ...
    if (url.includes("/file?") && opts && opts.method === "POST")
      return [{ code: 0, data: { fid: "DEST1" } }];
    if (url.includes("/file/sort"))
      return [{ code: 0, data: { list: [{ fid: "f1", file_name: "a.iso" }] } }]; // listFolder
    if (url.includes("/share?") && opts && opts.method === "POST") {
      return [{ code: 0, data: { share_url: "https://pan.quark.cn/s/Share01", passcode: "" } }];
    }
    return [{}, 200, {}];
  });
  const r = await quark.transfer({
    cookie: "ck",
    pwdId: "p",
    passcode: "",
    makeShare: true,
...
test("searchFiles：按关键词过滤第一层并保留 size/dir", async () => {
  ...
  const r = await quark.searchFiles("cookie", "0", "光芒");
...
test("trashFiles：POST /file/delete action_type=2 软删", async () => {
  ...
  global.fetch = makeFetch((u, opts) => {
    if (u.includes("/file/delete")) {
      sent = JSON.parse(opts.body);
      return [{ code: 0, data: {} }];
    }
    return [{ code: 0 }];
  });
  await quark.trashFiles("cookie", ["f1", "f2"]);
