...
function buildPrompt(gameName, opts = {}) {
  const en = opts.englishName || "无";
  let prompt = `请完成以下任务。用户已提供该游戏的网盘资源，因此这款游戏必然真实存在，请直接客观描述，不要质疑、不要写免责声明。\n`;
  prompt += `1. 先联网搜索游戏"${gameName}"（英文原名：${en}）的真实公开资料（Wikipedia 英文页、Metacritic、官方新闻稿、权威媒体测评，多源交叉验证），再据此撰写 80-110字 中文介绍（最少 50 字，最多 120 字）。评分、销量、奖项等数字必须是搜索到的真实数据；搜不到就不写该项，绝对不许猜测或编造。严禁返回"疑似虚构""无法确认""经核实无真实公开资料""请勿轻信""非官方渠道"等免责声明，否则视为错误输出。\n`;
  prompt += `2. 必须包含：开发商、发行年份、类型标签、核心玩法一句话；搜到则必加：Metacritic评分（XX分）、全球销量、权威奖项、一句权威评价收尾。结构模板：《游戏中文名》是<开发商>于<年份>推出的<类型标签>，<核心玩法>。<最突出的特色或创新点>。Metacritic评分<XX>分，全球销量超<XXX>万，<奖项或权威评价>。\n`;
  prompt += `3. 禁止事项：禁止以"该游戏"开头（直接用书名号《》起头）；禁止写"支持中文"；禁止推测评分（没搜到 Metacritic 分数就不写）；禁止罗列平台（不要写"登陆 PS4/Xbox/PC/Switch"这类平台枚举）；禁止堆砌空泛形容词（"精美画面""极致体验""沉浸式""身临其境"一律不写）。\n`;
  prompt += `4. 若你了解该游戏安装包大致大小（如"30.7G"）请一并给出，不确定则写"未抓取到"（不要尝试打开网盘链接读取，以你的已知公开信息为准）。\n`;
...
async function aiDescribe(gameName, rawLine, opts = {}) {
  ...
  try {
    ...
    if (r1.badIntro) {
      const retryPrompt = `请为游戏"${gameName}"（英文：${opts.englishName || "无"}）联网搜索真实公开资料（Wikipedia、Metacritic、官方新闻稿、权威测评，多源交叉验证），输出 80-110字 中文客观介绍（最少 50 字，最多 120 字）。必须包含开发商、发行年份、类型标签、核心玩法；搜到则加 Metacritic评分、全球销量、权威奖项，搜不到的数字一律不写、不许猜测。禁止以"该游戏"开头（直接用《》起头）、禁止写"支持中文"、禁止推测评分、禁止罗列平台、禁止堆砌"精美画面""极致体验""沉浸式"等空泛形容词，不要编造、不要写免责声明。严格按格式输出：\n介绍：<客观介绍>`;
      try {
