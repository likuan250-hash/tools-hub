...
async function exchangeCode(code) {
  const c = getConfig();
  const p = new URLSearchParams({ grant_type: 'authorization_code', code, client_id: c.clientId, client_secret: c.clientSecret, redirect_uri: c.redirectUri });
...
async function refresh(refreshToken) {
  const c = getConfig();
  const p = new URLSearchParams({ grant_type: 'refresh_token', refresh_token: refreshToken, client_id: c.clientId, client_secret: c.clientSecret });
  const res = await fetch(`${OAUTH_BASE}/token?${p.toString()}`);
  const data = await res.json();
...
function syncJar() {
  ...
  cleanCookie = Object.entries(jar).map(([k, v]) => `${k}=${v}`).join('; ');
}
...
const ESSENTIAL_COOKIES = ['BDUSS', 'BDUSS_BFESS', 'STOKEN', 'STOKEN_BFESS', 'BDCLND', 'BAIDUID', 'PANPSC'];
...
async function getBdstoken() {
  ...
  if (data.errno !== 0 || !data.result || !data.result.bdstoken) {
    ...
    if (data.errno === -6) {
      ...
      const names = Object.keys(cookieJar).filter((k) => /^(BDUSS|STOKEN|BDCLND|BAIDUID|PANPSC|PTOKEN|HOSUPPORT)/.test(k));
      const hasBduss = !!(cookieJar['BDUSS'] || cookieJar['BDUSS_BFESS']);
      const hasStoken = !!(cookieJar['STOKEN'] || cookieJar['STOKEN_BFESS']);
      hint = `（百度登录态失效 errno=-6。已发核心凭证 BDUSS=${hasBduss ? '有' : '无'} STOKEN=${hasStoken ? '有' : '无'}; `
...
async function verifyCookie(cookieStr) {
  ...
  const headerFull = Object.entries(jar).map(([k, v]) => `${k}=${v}`).join('; ');
  ...
  const tryOnce = async (header) => {
    try {
      ...
      const res = await fetch(url, { headers: Object.assign({}, BASE_HEADERS, { Cookie: header }), signal: AbortSignal.timeout(8000) });
      const data = await res.json();
...
async function verify(surl, pwd, bdstoken) {
  const qs = new URLSearchParams({ surl, bdstoken, t: String(Date.now()), channel: 'chunlei', web: '1', clienttype: '0' });
  const res = await fetch(`${PAN}/share/verify?${qs}`, {
    method: 'POST',
    headers: reqHeaders({ 'Content-Type': 'application/x-www-form-urlencoded', Referer: `${PAN}/share/init?surl=${surl}` }),
    body: new URLSearchParams({ pwd, vcode: '', vcode_str: '' }).toString(),
  });
  const data = await res.json();
  if (data.errno !== 0) throw new Error('提取码错误(errno=' + data.errno + '),请确认分享提取码是否正确');
...
async function getShareList(surl, pwd) {
  ...
  if (!shareid || !uk || !list.length) {
    throw new Error('未能从分享页解析出文件列表(可能链接是分享者主页而非文件分享,或需先点"提取文件";或 Cookie/BDUSS 已失效)');
  }
...
async function transfer(shareid, uk, fsidList, destPath) {
  ...
  const bdstoken = await getBdstoken();
  const qs = new URLSearchParams({ shareid: String(shareid), from: String(uk), bdstoken, channel: 'chunlei', web: '1', clienttype: '0' });
  ...
  if (data.errno !== 0 && data.errno !== 4 && data.errno !== 12) throw new Error('转存失败 errno=' + data.errno + ' ' + JSON.stringify(data).slice(0, 160));
  const file_list = (data.file_list || []).map((f) => ({ fs_id: f.fs_id, path: f.path, server_filename: f.server_filename, size: f.size }));
  const duplicated = (data.duplicated && data.duplicated.list || []).map((f) => ({ fs_id: f.fs_id, path: f.path, server_filename: f.server_filename, size: f.size }));
  return {
...
async function ensureDir(destPath) {
  const bdstoken = await getBdstoken();
  const lp = new URLSearchParams({ order: 'time', desc: '1', showempty: '0', web: '1', page: '1', num: '1000', dir: destPath, bdstoken });
  ...
  if (cj.errno !== 0) throw new Error('创建转存目录失败 errno=' + cj.errno + ' ' + JSON.stringify(cj).slice(0, 120));
...
async function listDir(dirPath) {
  const bdstoken = await getBdstoken();
  const lp = new URLSearchParams({ order: 'time', desc: '1', showempty: '0', web: '1', page: '1', num: '1000', dir: dirPath, bdstoken });
  const fl = await fetch(`${PAN}/api/list?${lp}`, { headers: reqHeaders() });
  const flj = await fl.json();
  if (flj.errno !== 0) throw new Error('列目录失败 errno=' + flj.errno + ' ' + JSON.stringify(flj).slice(0, 120));
  return (flj.list || []).map((f) => ({ fs_id: f.fs_id, server_filename: f.server_filename, isdir: f.isdir, path: f.path }));
}
...
async function createShare(fsIdList, period, password) {
  ...
  const bdstoken = await getBdstoken();
  const qs = new URLSearchParams({ channel: 'chunlei', bdstoken, clienttype: '0', app_id: '250528', web: '1' });
  ...
  const data = await res.json();
  if (data.errno !== 0) throw new Error('生成分享失败 errno=' + data.errno + ' ' + JSON.stringify(data).slice(0, 160));
...
async function checkSession() {
  syncJar();
  if (!cookieHeader()) { lastCheckError = 'no_session_cookie'; return false; }
  try {
    const fields = encodeURIComponent('["bdstoken","token","uk","isdocuser","servertime"]');
    const url = `${PAN}/api/gettemplatevariable?clienttype=0&app_id=38824127&web=1&fields=${fields}&_t=${Date.now()}`;
    ...
    return ok;
  } catch (e) { lastCheckError = 'net_error:' + (e.name || e.message || 'unknown'); return false; }
}

// 返回最近一次 checkSession 的失败原因(供前端诊断展示)
function getLastCheckError() { return lastCheckError; }
