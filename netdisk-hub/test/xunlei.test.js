...
before(() => {
  ...
  restoreFetch = () => { global.fetch = orig; };
...
test('parseSurl：补全为完整迅雷分享链接（支持完整 URL / ?s= 查询 / 纯短码）', () => {
  assert.strictEqual(xunlei.parseSurl('https://pan.xunlei.com/s/AbC123'), 'https://pan.xunlei.com/s/AbC123');
...
test('getShareInfo：解析 files 与 pass_code_token', async () => {
  global.fetch = makeFetch((url) => {
    if (url.includes('/share')) return [{ data: { files: [{ id: 'fid1', name: 'a.iso', size: 9 }], pass_code_token: 'PT' } }];
    return [{}, 200, {}];
  });
...
test('createShare：返回带提取码链接', async () => {
  global.fetch = makeFetch(() => [{ data: { share_url: 'https://pan.xunlei.com/s/Abcde', pass_code: 'p1', share_id: 'S1' } }]);
...
test('findFolder：按名精确匹配文件夹', async () => {
  global.fetch = makeFetch((url) => {
    if (url.includes('/files')) return [{ files: [{ id: 'X1', name: '游戏', kind: 'drive#folder' }, { id: 'X2', name: '音乐', kind: 'drive#file' }] }];
    return [{}, 200, {}];
  });
...
test('transfer：完整编排（指定目标目录 + 生成分享）', async () => {
  global.fetch = makeFetch((url, opts) => {
    ...
    if (url.includes('/tasks/')) return [{ progress: 100, params: { trace_file_ids: '{"a":"fid1"}' } }];
    if (url.includes('/share') && isPost) return [{ data: { share_url: 'https://pan.xunlei.com/s/Abcde', pass_code: 'p1', share_id: 'S1' } }];
    if (url.includes('/share')) return [{ data: { files: [{ id: 'fid1', name: 'a.iso', size: 9 }], pass_code_token: 'PT' } }];
    return [{}, 200, {}];
  });
  const r = await xunlei.transfer({ link: 'https://pan.xunlei.com/s/SH1', pwd: '', makeShare: true, destFolderId: 'DEST', destFolderName: '游戏' });
...
test('transfer：未指定目录时按名查找「游戏」', async () => {
  global.fetch = makeFetch((url, opts) => {
    ...
    if (url.includes('/share/restore')) return [{ restore_task_id: 'RT1' }];
    if (url.includes('/tasks/')) return [{ progress: 100, params: { trace_file_ids: '{"a":"fid1"}' } }];
    if (url.includes('/share') && isPost) return [{ data: { share_url: 'https://pan.xunlei.com/s/Abcde', pass_code: '', share_id: 'S1' } }];
    if (url.includes('/share')) return [{ data: { files: [{ id: 'fid1', name: 'a.iso', size: 9 }], pass_code_token: 'PT' } }];
    // listFiles：用于 findFolder('游戏')
    if (url.includes('/files')) return [{ files: [{ id: 'GAME', name: '游戏', kind: 'drive#folder' }] }];
    return [{}, 200, {}];
  });
  const r = await xunlei.transfer({ link: 'https://pan.xunlei.com/s/SH1', pwd: '', makeShare: false });
  assert.strictEqual(r.destPath, '/游戏');
