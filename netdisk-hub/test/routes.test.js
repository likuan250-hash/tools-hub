...
before(async () => {
  ...
  const ctx = {
    ...
    getVersion: () => '1.3.30',
    PORT: 3000,
    process, path, fs, __dirname: __dirname,
  };
...
test('POST /api/transfer/batch：批量成功返回各结果', async () => {
  ...
  const { status, body } = await send('POST', '/api/transfer/batch', { jobs: [{ provider: 'quark' }, { provider: 'baidu' }] });
  assert.strictEqual(status, 200);
...
test('/api/health：无配置且 fatal 返回 503', async () => {
  ...
  const ctxUnhealthy = {
    store,
    logger: { info() {}, warn() {}, error() {} },
    baidu: { getConfig: () => ({ appDir: '/x' }), getLastCheckError: () => '', getCookie: () => null, listSubfolders: async () => [] },
    ...
    extractSurl: (s) => s, isValidShareLink: () => true, refreshPings: () => {},
    pingCache: { baidu: false, quark: false, xunlei: false },
    getServerState: () => ({ healthy: false, fatalCount: 3 }),
    getVersion: () => '1.3.30', PORT: 3000, process, path, fs, __dirname: __dirname,
  };
  const app2 = express();
