...
test('parseSearchResults 解析 --flat-playlist --dump-json 的 NDJSON 多行输出', () => {
  const t = make();
  const raw = [
    JSON.stringify({ id: 'v1', title: 'A - Launch Trailer', channel: 'PlayStation', duration: 120 }),
...
test('parseSearchResults 兼容 entries 形态 / 裸 id url / 非法输入', () => {
  const t = make();
  const withEntries = JSON.stringify({ entries: [{ id: 'e1', title: 'E1' }, null, { title: '无 id' }] });
...
test('normalizeEntry 识别认证频道并回退 uploader 字段', () => {
  const t = make();
  const e = t.normalizeEntry({ id: 'x', title: 'T', uploader: 'Bandai Namco', channel_is_verified: true });
...
test('normalizeText / nameTokens 归一化与分词', () => {
  assert.equal(normalizeText('ELDEN RING: Shadow of the Erdtree'), 'elden ring shadow of the erdtree');
...
test('scoreChannel 按开发商 > 发行商 > 平台方 > 聚合号分档，二创号负分', () => {
  ...
  assert.deepEqual(t.scoreChannel('FromSoftware', 'Elden Ring', { developer: 'FromSoftware' }),
    { score: 45, tier: 'developer' });
  // 未传开发商时，发行商名单命中
  assert.equal(t.scoreChannel('FromSoftware', 'Elden Ring').tier, 'publisher');
  assert.equal(t.scoreChannel('Bandai Namco Entertainment America', 'Elden Ring').tier, 'publisher');
...
test('scoreCandidate 综合打分：官方 Launch Trailer 远高于二创搬运', () => {
  ...
  const official = t.scoreCandidate(
    { id: 'a', title: 'ELDEN RING - Official Launch Trailer', channel: 'Bandai Namco Entertainment America', duration: 180 },
    'Elden Ring',
  );
  const reaction = t.scoreCandidate(
    { id: 'b', title: 'ELDEN RING Launch Trailer REACTION!!', channel: 'Random Reaction Channel', duration: 900 },
    'Elden Ring',
  );
...
test('pickBest 传入 developer 时优先开发商官方频道', () => {
  const t = make();
  const items = [
    { id: 'pub', title: 'ELDEN RING - Launch Trailer', channel: 'Bandai Namco Entertainment America', duration: 120 },
...
test('findDownloaded 按基名匹配、优先 mp4、忽略中间产物', () => {
  assert.equal(make({ entries: ['base.webm', 'base.mp4', 'other.mp4'] }).findDownloaded('dir', 'base'), 'base.mp4');
  assert.equal(make({ entries: ['base.webm'] }).findDownloaded('dir', 'base'), 'base.webm');
  assert.equal(make({ entries: ['base.mp4.part', 'base.ytdl'] }).findDownloaded('dir', 'base'), null);
...
test('searchTrailer 用注入的 yt-dlp 绝对路径执行，命中后发 trailer_search 事件', async () => {
  ...
  const stdout = [
    ...
    JSON.stringify({ id: 'v1', title: 'ELDEN RING - Official Launch Trailer', channel: 'Bandai Namco Entertainment America', duration: 180 }),
  ].join('\n');
...
test('searchTrailer 无候选 / 候选全不达标 / 命令异常均安全返回 null', async () => {
  ...
  const events = [];
  const onlyBad = JSON.stringify({ id: 'b', title: 'ELDEN RING Launch Trailer REACTION!!', channel: 'Reaction Guy', duration: 900 });
...
test('download 区分 yt-dlp 失败与产出文件缺失', async () => {
  ...
  const r1 = await fail.download('x', 'dir', { ytDlp: true, ffmpeg: true }, { info: { id: 'v', title: 'T', url: 'u' } });
  assert.equal(r1.ok, false);
  assert.equal(r1.reason, 'yt-dlp-failed');

  const missing = make({ plan: { code: 0 }, entries: [] });
  const r2 = await missing.download('x', 'dir', { ytDlp: true, ffmpeg: true }, { info: { id: 'v', title: 'T', url: 'u' } });
...
test('probeResolution 对低于 1080p 的视频标记未达标', async () => {
  ...
  const r = await t.probeResolution('a.mp4', { emit: (type, step, msg, ok, detail) => events.push({ type, ok, detail }) });
  assert.equal(r.ok, true);
...
test('resolveProxyUrl 大小写变体都识别（https_proxy / Http_Proxy 等）', () => {
  assert.equal(make({ env: { https_proxy: 'http://127.0.0.1:7990/' } }).resolveProxyUrl(), 'http://127.0.0.1:7990');
  assert.equal(make({ env: { HTTPS_PROXY: 'http://10.0.0.1:8080' } }).resolveProxyUrl(), 'http://10.0.0.1:8080');
});
...
test('resolveProxyUrl 尊重 NO_PROXY：* 与 localhost 均绕过代理', () => {
  assert.equal(make({ env: { HTTPS_PROXY: 'http://127.0.0.1:7990/', NO_PROXY: '*' } }).resolveProxyUrl(), '');
...
test('buildSearchArgs / buildDownloadArgs 本身不带 --proxy（保持纯净，由 withProxyArgs 叠加）', () => {
  ...
  assert.ok(!t.buildDownloadArgs('https://youtu.be/v', 'dir/out.mp4', { ffmpeg: true }).includes('--proxy'));
});
