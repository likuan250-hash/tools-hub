...
- [x] Bing 是否需要代理？（结论：国内可直连，netFetch 直连优先、失败一次后降级代理；走代理反而被 geo 重定向到 bing.com?cc=US 破坏中文布局）
- [x] Bing 反爬缓解措施（UA + Accept-Language:zh-CN + Referer + 600ms 节流 + 最多 1 次退避重试 + 单 run 12 次熔断）
- [x] Steam library_hero 尺寸不达标的处理策略（拆成 strict 达标档 + lowres 降级档两个 source id，分列链首与链尾；lowres 命中实测尺寸算 degraded）
- [x] `discoverViaDuckDuckGo` 是否彻底删除（已彻底删除，连同 5 个只服务它的解析器，避免超时拖累与死代码误导）
