// biliup-hub/test/app.renderer.test.js
// 渲染层回归测试：在沙箱内（无 Electron）用轻量 DOM stub + vm 真实执行 public/app.js / index.html
// 图标系统 / 执行按钮 setExec / 统一 Toast / 按钮结构 / 零回归（厚玻璃/焦点环/减弱动效）。
// 与 renderer/test/regression-stuck-initializing.test.js 同源的 stub 范式，按需增强。
//
// 运行：cd biliup-hub && node --test test/app.renderer.test.js
...
// 构造 vm 上下文 + 最小 DOM。nullToastHost=true 时 getElementById('toastHost') 返回 null（模拟首次创建）。
function makeContext(opts = {}) {
  ...
  ctx.$ = (id) => documentStub.getElementById(id); // 供从 IIFE 中抽取的孤立函数（如 biliup 的 toast 使用 $）
  ctx.console = console;
...
// 从 index.html 提取「统一图标系统」内联脚本块（window.ICONS = { ... hydrateIcons(document);）。
...
// 构造一个带 .bx-label 子节点的按钮 El（供 setExec 测试）。
function makeButtonEl() {
  const btn = new El("button");
  const ico = new El("svg"); ico.classList.add("bx-ico");
  const label = new El("span"); label.classList.add("bx-label"); label.textContent = "投稿";
  btn.appendChild(ico);
...
test("图标系统：window.ico('check') 返回含 <svg class=\"app-ico\" 的字符串", () => {
  ...
  const out = ctx.window.ico("check");
  assert.ok(/<svg class="app-ico"/.test(out), "ico('check') 应含 <svg class=\"app-ico\"");
});
test("图标系统：未知图标名 ico('nope') 优雅返回空串（不抛错、不产生破损 SVG）", () => {
  const { ctx } = makeContext();
  vm.runInContext(extractIconBlock(indexHtml), ctx);
...
test("图标系统：hydrateIcons(root) 能把 [data-ico] 元素填成 <svg>", () => {
  ...
  assert.ok(/<svg class="app-ico"/.test(child.innerHTML), "data-ico=check 应被填充为 svg");
});
test("图标系统：window.ICONS / window.ico / window.hydrateIcons 三模块 index.html 逐字节一致（md5）", () => {
  const a = iconBlockMd5(indexHtml);
  const b = iconBlockMd5(fs.readFileSync(path.join(__dirname, "..", "..", "kdocs-tool", "public", "index.html"), "utf8"));
  const c = iconBlockMd5(fs.readFileSync(path.join(__dirname, "..", "..", "netdisk-hub", "public", "index.html"), "utf8"));
  assert.strictEqual(a, b, "biliup 与 kdocs 图标脚本块应逐字节一致");
  assert.strictEqual(b, c, "kdocs 与 netdisk 图标脚本块应逐字节一致");
});
...
test("setExec(true)：加 .is-loading、disabled=true、bx-label 变「执行中…」", () => {
  ...
  assert.ok(btn.classList.contains("is-loading"), "应含 .is-loading");
  assert.strictEqual(btn.disabled, true, "disabled 应为 true");
  assert.strictEqual(btn.getAttribute("aria-busy"), "true", "aria-busy 应为 true");
  assert.strictEqual(btn.querySelector(".bx-label").textContent, "执行中…", "bx-label 应变为执行中…");
});
test("setExec(false)：复原 .is-loading 移除、disabled=false、bx-label 恢复原文案", () => {
  ...
  assert.ok(!btn.classList.contains("is-loading"), "应移除 .is-loading");
  assert.strictEqual(btn.disabled, false, "disabled 应为 false");
  assert.strictEqual(btn.getAttribute("aria-busy"), null, "aria-busy 应被移除");
  assert.strictEqual(btn.querySelector(".bx-label").textContent, "投稿", "bx-label 应恢复为原文案");
});
test("setExec：异步流程抛异常时 finally 必调用 setExec(btn,false)，按钮绝不卡在 loading", async () => {
  const { ctx } = makeContext();
  vm.runInContext("var setExec = (" + extractFunction(appSrc, "setExec") + ");", ctx);
  // 复刻 submit()/runAuto()/batchBtn 的真实用法：try{ await ... } finally { setExec(btn, false) }
  vm.runInContext(
  ...
  assert.strictEqual(btn.disabled, false, "finally 后 disabled 应复位");
  assert.strictEqual(btn.querySelector(".bx-label").textContent, "投稿", "finally 后文案应复原");
});

// ───────────────────────── 3) 统一 Toast ─────────────────────────
test("toast(ok)：在 #toastHost 下生成玻璃 .toast-host / .toast 子节点，含正确文案与 check 图标", () => {
  const { ctx, ids, documentStub } = makeContext({ nullToastHost: true });
  vm.runInContext(extractIconBlock(indexHtml), ctx); // 提供 ico()
  vm.runInContext("var toast = (" + extractFunction(appSrc, "toast") + ");", ctx);
  ctx.toast("参数已保存", "ok");
  // toast 首次创建 host 并挂到 body
  ...
  assert.strictEqual(toastEl._sinks[".toast-msg"].textContent, "参数已保存", "toast 文案应正确");
...
// ───────────────────────── 4) 按钮结构（无 emoji 残留） ─────────────────────────
test("按钮结构：#submitBtn(.btn-exec) 含 .bx-ico(SVG) + .bx-label", () => {
  const re = /<button class="btn-exec" id="submitBtn"[\s\S]*?<svg class="bx-ico"[\s\S]*?<span class="bx-label">/;
  assert.ok(re.test(indexHtml), "submitBtn 应含 bx-ico 与 bx-label");
});
test("按钮结构：.icon-btn(#clearBtn) 含 .ib-ico + .ib-label", () => {
  const re = /<button class="icon-btn" id="clearBtn"[\s\S]*?class="ib-ico"[\s\S]*?class="ib-label">/;
  assert.ok(re.test(indexHtml), "clearBtn 应含 ib-ico 与 ib-label");
});
...
test("零回归：.modal.glass 厚玻璃 backdrop-filter: blur(40px) saturate(2.0)", () => {
  assert.ok(/\.modal\.glass\s*\{[\s\S]*?backdrop-filter:\s*blur\(40px\)\s*saturate\(2\.0\)/.test(indexHtml),
    "应包含 .modal.glass { ... backdrop-filter: blur(40px) saturate(2.0)");
});
test("零回归：:focus-visible 键盘焦点环门禁存在", () => {
  assert.ok(/:focus-visible\s*\{/.test(indexHtml), "应包含 :focus-visible 规则");
});
test("零回归：prefers-reduced-motion 减弱动效门禁存在", () => {
  assert.ok(/prefers-reduced-motion/.test(indexHtml), "应包含 prefers-reduced-motion 媒体查询");
});

// ───────────────────────── 6) 加载安全：IIFE 真实执行不崩溃 ─────────────────────────
test("加载安全：public/app.js(IIFE) 在沙箱中真实执行不抛异常（含 status-luxe/图标/helpers）", () => {
  const { ctx } = makeContext();
  vm.runInContext(fs.readFileSync(path.join(PUBLIC, "..", "public", "status-luxe.js"), "utf8"), ctx);
  vm.runInContext(extractIconBlock(indexHtml), ctx);
  for (const f of ["select-preserve.js", "coerce-int.js", "season-align.js"]) {
    vm.runInContext(fs.readFileSync(path.join(PUBLIC, f), "utf8"), ctx);
  }
  assert.doesNotThrow(() => { vm.runInContext(appSrc, ctx); }, "IIFE 不应抛异常");
});
