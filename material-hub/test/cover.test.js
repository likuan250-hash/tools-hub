...
function fakeFs() {
  return {
    ...
    unlinked: [],
    writeFileSync(p, buf) {
      this.written.push({ path: p, bytes: buf.length });
    },
    unlinkSync(p) {
...
test("buildWallhavenApiUrl 必须用 categories=100（general），绝不能是 010（anime）", () => {
  ...
  assert.equal(
    ...
    WALLHAVEN_API +
      "?q=Just%20Cause%204&atleast=1280x720&categories=100&purity=100&sorting=relevance",
  );
});
...
test("buildWallhavenApiUrl 四个参数均可由 opts 覆盖（不再硬编码在拼接串里）", () => {
  ...
  const c2 = new CoverFetcher({
    fetch: fakeFetch({}),
    fs: fakeFs(),
    wallhaven: { categories: "111", sorting: "random" },
  });
...
test("parseWallhavenResults 取 data[].path 并挡掉明确不达标项", () => {
  ...
  const json = {
    data: [
      {
        path: "https://w.wallhaven.cc/full/ab/wallhaven-1.jpg",
        dimension_x: 1920,
        dimension_y: 1080,
      },
      {
        path: "https://w.wallhaven.cc/full/cd/wallhaven-2.jpg",
        dimension_x: 1280,
        dimension_y: 720,
      },
...
test("parseBingImageResults 从 <a m=> 提取原图直链 / 来源页 / 标题，并反转义", () => {
  const html = [
    bingItemHtml({
      ...
      t: "Just Cause 4 Wallpaper",
    }),
    bingItemHtml({
      murl: "https://images.alphacoders.com/136/1360000.jpg",
      purl: "https://alphacoders.com/x",
      t: "Just Cause 4",
    }),
  ...
  assert.equal(
    parseBingImageResults(bingItemHtml({ murl: "//x.com/a.jpg" }))[0].murl,
    "https://x.com/a.jpg",
  );
...
test("filterBingCandidates 只保留指定域名（purl host）且相关的候选，并去重截断", () => {
  const items = [
    {
      murl: "https://cdn.4kwallpapers.com/jc4.jpg",
      purl: "https://4kwallpapers.com/jc4",
      title: "Just Cause 4",
    },
    {
      murl: "https://images.alphacoders.com/136/1360000.jpg",
    ...
    {
      murl: "https://cdn.4kwallpapers.com/jc4-2.jpg",
      purl: "https://4kwallpapers.com/jc4-2",
      title: "Just Cause 4 Reloaded",
    },
    {
      murl: "https://cdn.4kwallpapers.com/persona.jpg",
      purl: "https://4kwallpapers.com/persona",
      title: "Persona 4",
    }, // 不相关
    {
      murl: "https://cdn.4kwallpapers.com/jc4.jpg",
...
test("isBingItemRelevant 标题 / murl slug / purl slug 任一自证相关即通过", () => {
  ...
  assert.equal(
    isBingItemRelevant(
      { murl: "https://x/just-cause-4.jpg", purl: "https://4kwallpapers.com/jc4", title: "X" },
      q,
    ),
    true,
  );
  assert.equal(
    isBingItemRelevant(
      { murl: "https://x/foo.jpg", purl: "https://4kwallpapers.com/just-cause-4", title: "X" },
      q,
    ),
    true,
    "purl slug 命中",
  ...
  assert.equal(
    isBingItemRelevant(
      { murl: "https://x/foo.jpg", purl: "https://a.com/b", title: "Just Cause 4 Wallpaper" },
      q,
    ),
    true,
    "标题命中",
  ...
  assert.equal(
    isBingItemRelevant(
      { murl: "https://x/persona.jpg", purl: "https://a.com/b", title: "Persona 4" },
      q,
    ),
    false,
    "三者都不相关",
  ...
  assert.equal(
    isBingItemRelevant({ murl: "https://x/foo.jpg", purl: "https://a.com/b", title: "X" }, []),
    false,
    "无 token 一律不相关",
  );
});
...
test("looksLikeBingBlockPage 识别拦截/空页，但完整 0 结果页不算拦截", () => {
  ...
  const full =
    "<html><body>" +
    bingItemHtml({ murl: "https://x/a.jpg", purl: "https://x/a", t: "A" }) +
    "zzz".repeat(400) +
    "</body></html>";
...
test("buildBingImageUrl 单站 / 多站 OR / 尺寸过滤（qft 绝不 encodeURIComponent）", () => {
  ...
  assert.ok(
    sites.every((s) => multi.includes(encodeURIComponent("site:" + s))),
    "每个站点都在",
  );
});
...
test("buildSteamCdnUrl / buildSteamCdnCandidates 拼官方图地址，非数字 appid 返回空", () => {
  const c = new CoverFetcher({ fetch: fakeFetch({}), fs: fakeFs() });
  assert.equal(
    c.buildSteamCdnUrl("123", "library_hero_2x.jpg"),
  ...
  assert.deepEqual(c.buildSteamCdnCandidates("123", "lowres"), [
    STEAM_CDN_BASE + "/123/library_hero.jpg",
  ]);
    ...
    pickRelevantSteamAppId(
      {
        total: 2,
        items: [
          { id: 111, name: "Persona 4" },
          { id: 222, name: "Just Cause 4" },
        ],
      },
      normalizeTokens("just cause 4"),
    ),
    "222",
  );
  assert.equal(
    pickRelevantSteamAppId(
      { total: 1, items: [{ id: 1, name: "Persona 4" }] },
      normalizeTokens("just cause 4"),
    ),
    "",
    "无相关项返回空",
...
test("resolveSteamAppId 入参优先 → 缓存命中 → storesearch 反查", async () => {
  // 用离线库未收录的游戏名，确保走 storesearch 网络路径（离线库收录的会直接离线命中）
  const game = "Mystery Game 8888";
  const storeUrl =
  ...
  const c0 = makeCover({});
  assert.equal(await c0.resolveSteamAppId(game, { steamAppId: "999" }), "999");

  // 走 storesearch
  const c = makeCover({ [storeUrl]: { json: { total: 1, items: [{ id: 123, name: game }] } } });
  assert.equal(await c.resolveSteamAppId(game), "123");

  // 缓存命中：再问一次不打第二个请求
  const calls = [];
  const c2 = makeCover(
    { [storeUrl]: { json: { total: 1, items: [{ id: 123, name: game }] } } },
    null,
    calls,
  );
  await c2.resolveSteamAppId(game);
  ...
  assert.equal(await c3.resolveSteamAppId(game), "");

  // lookup=false 关闭反查
  const calls4 = [];
  const c4 = makeCover({}, null, calls4);
  assert.equal(await c4.resolveSteamAppId(game, { lookup: false }), "");
...
test("fromSteamCdn strict 档强制尺寸门槛；lowres 档跳过门槛但如实标 degraded", async () => {
  const fs = fakeFs();
  const strictUrls = [
    "https://cdn.akamai.steamstatic.com/steam/apps/123/capsule_616x353_2x.jpg",
    "https://cdn.akamai.steamstatic.com/steam/apps/123/capsule_616x353.jpg",
    "https://cdn.akamai.steamstatic.com/steam/apps/123/header.jpg",
  ];
  ...
  const c1 = makeCover(
    { [strictUrls[0]]: { buf: jpegBuf(1920, 1080) }, [strictUrls[1]]: { buf: jpegBuf(640, 480) } },
    fs,
  );
...
test("PNG 候选先按真实格式落盘再转码成 封面.jpg，绝不改名冒充", async () => {
  ...
  const c = new CoverFetcher({
    fetch: fakeFetch({ [png]: { buf: pngBuf(3840, 2160) } }),
    fs,
    probe,
  });
...
test("第 2 级 wallhaven 端到端：API → 直链 → 下载 → 校验 → 落盘", async () => {
  const api =
    "https://wallhaven.cc/api/v1/search?q=Elden%20Ring&atleast=1280x720&categories=100&purity=100&sorting=relevance";
...
test("fetchCover 按新规范顺序降级：steam-cdn 无 appid 跳过 → wallhaven 命中", async () => {
  ...
  assert.deepEqual(r.tried, ["4kwallpapers", "wallhaven"]);
...
test("fetchCover 缺少入参的来源被跳过（不计入 tried）", async () => {
  ...
  assert.deepEqual(r.tried, ["4kwallpapers", "wallhaven", "reddit", "alphacoders"]);
});
...
test("第 4 级用户指定 URL：默认排在第 4 位，userUrlFirst 可提前", async () => {
  ...
  assert.deepEqual(r1.tried, ["4kwallpapers", "wallhaven", "reddit", "user"]);
...
test("HTML 错误页伪装成图片时被识别并拒绝", async () => {
  const c = makeCover({
    "https://a.com/x.jpg": { buf: Buffer.from("<!DOCTYPE html><html>404 not found</html>") },
  });
...
test("parseSteamSearchAppId / parseSteamAppName 解析 Steam 两步反查响应", () => {
  assert.equal(
    parseSteamSearchAppId({ total: 1, items: [{ id: 1325200, name: "仁王２ Complete Edition" }] }),
    "1325200",
  );
...
test("resolveEnglishTitle：中文名经维基百科反查拿到英文名并缓存", async () => {
  const zhSearchUrl =
    ...
    encodeURIComponent("仁王3") +
    "&format=json&srlimit=5";
  const llUrl =
    "https://zh.wikipedia.org/w/api.php?action=query&prop=langlinks&lllang=en&pageids=123&format=json&lllimit=1";
  const enPagesUrl =
    "https://en.wikipedia.org/w/api.php?action=query&prop=pageprops&titles=" +
    encodeURIComponent("Nioh 3") +
    "&format=json";
  ...
  const c = makeCover(
    {
      ...
      [enPagesUrl]: {
        json: { query: { pages: { "-1": { pageprops: { wikibase_item: "Q12345" } } } } },
      },
      [wdUrl]: {
        json: {
          entities: {
            Q12345: { claims: { P1733: [{ mainsnak: { datavalue: { value: "1325200" } } }] } },
          },
        },
      },
    },
...
test("resolveEnglishTitle：维基百科查不到 / 出错时退回空英文名，绝不报错", async () => {
  ...
  const zhUrl =
    "https://zh.wikipedia.org/w/api.php?action=query&list=search&srsearch=" +
    encodeURIComponent("虚构游戏名测试") +
    "&format=json&srlimit=5";
  const llUrl =
    "https://zh.wikipedia.org/w/api.php?action=query&prop=langlinks&lllang=en&pageids=999&format=json&lllimit=1";
  const c3 = makeCover({
  ...
  assert.deepEqual(await c4.resolveEnglishTitle("正当防卫4", { lookup: false }), {
    title: "",
    source: "none",
  });
  assert.equal(calls.length, 0);
...
test("fetchCover 不传 englishTitle 时自动经维基百科反查（端到端链路）", async () => {
  ...
  const c = makeCover((url) => {
    ...
    if (url === llUrl)
      return { json: { query: { pages: { 123: { langlinks: [{ "*": "Nioh 3" }] } } } } };
    if (url.includes("wallhaven.cc/api")) {
...
test("extractSlugFromUrl 剥掉目录/扩展名/分辨率尾巴/站点 id，但保留续作编号", () => {
  assert.equal(
    extractSlugFromUrl(
      "https://4kwallpapers.com/images/wallpapers/persona-4-revival-3840x2160-26747.jpg",
    ),
    "persona-4-revival",
  );
  assert.equal(
  ...
  assert.equal(
    extractSlugFromUrl("https://4kwallpapers.com/games/just-cause-4-4142.html"),
    "just-cause-4",
  );
...
test("extractTitleFromHtml 取 og:title 优先，退回 <title>", () => {
  assert.equal(
    extractTitleFromHtml(
      '<meta property="og:title" content="Just Cause 4 HD Wallpaper"><title>Wallpaper Abyss</title>',
    ),
...
test("isRelevantCandidate 必须判定实测错图为不相关（缺陷 4 回归锁）", () => {
  ...
  assert.equal(
    isRelevantCandidate("persona-4-revival", q),
    false,
    "persona-4-revival 只命中一个数字 4",
  );
  assert.equal(
    isRelevantCandidate("kagurabachi-key-art", q),
    false,
  ...
  assert.equal(
    isRelevantCandidate(
      extractSlugFromUrl(
        "https://4kwallpapers.com/images/wallpapers/just-cause-4-3840x2160-777.jpg",
      ),
      q,
    ),
    true,
  ...
  assert.equal(
    isRelevantCandidate(
      extractSlugFromUrl(
        "https://4kwallpapers.com/images/wallpapers/kagurabachi-key-art-3840x2160-26856.jpg",
      ),
      q,
    ),
    false,
...
test("isRelevantCandidate 主要 token 必须大部分命中，光命中数字不算数", () => {
  ...
  assert.equal(
    isRelevantCandidate("just-another-thing", jc4),
    false,
    "只命中 just，2 个实义词要求全中",
  );
...
test("isRelevantCandidate 系列共享前缀数字（40,000）不误放行前作", () => {
  ...
  assert.equal(
    isRelevantCandidate("warhammer 40,000 space marine anniversary edition", q),
    false,
    "周年纪念版无 2，拒绝",
  );
  assert.equal(isRelevantCandidate("warhammer 40,000 space marine 2", q), true);
  assert.equal(
    isRelevantCandidate("warhammer-40000-space-marine-2-3840x2160", q),
    true,
    "slug 数字连写也能命中续作标记",
  );
});
...
test("parseDuckDuckGoLinks 从 DDG 结果页提取目标站点详情页 URL", () => {
  const html = [
    '<a class="result__a" href="//duckduckgo.com/l/?uddg=' +
      encodeURIComponent("https://4kwallpapers.com/games/titan-quest-ii-game-17991.html") +
      '&rut=abc">Titan Quest II</a>',
    '<a href="//duckduckgo.com/l/?uddg=' +
      encodeURIComponent("https://4kwallpapers.com/games/black-desert-key-26805.html") +
      '&rut=def">Black Desert</a>',
    '<a href="//duckduckgo.com/l/?uddg=' +
      encodeURIComponent("https://example.com/other.html") +
      '&rut=ghi">Other</a>',
  ].join("\n");
  const links = parseDuckDuckGoLinks(html, "4kwallpapers.com");
  assert.deepEqual(links, [
    "https://4kwallpapers.com/games/titan-quest-ii-game-17991.html",
    "https://4kwallpapers.com/games/black-desert-key-26805.html",
  ]);
  assert.deepEqual(parseDuckDuckGoLinks("", "4kwallpapers.com"), []);
});

test("parse4kwallpapersDirect 提取 ≥1920×1080 直链并过滤小图", () => {
  const html = [
    "https://4kwallpapers.com/images/wallpapers/titan-quest-ii-game-1280x720-17991.jpg",
    "https://4kwallpapers.com/images/wallpapers/titan-quest-ii-game-1920x1080-17991.jpg",
    "https://4kwallpapers.com/images/wallpapers/titan-quest-ii-game-3840x2160-17991.jpg",
    "https://4kwallpapers.com/images/wallpapers/titan-quest-ii-game-2560x1440-17991.jpg",
  ].join(" ");
  assert.deepEqual(parse4kwallpapersDirect(html), [
    "https://4kwallpapers.com/images/wallpapers/titan-quest-ii-game-1920x1080-17991.jpg",
    "https://4kwallpapers.com/images/wallpapers/titan-quest-ii-game-3840x2160-17991.jpg",
    "https://4kwallpapers.com/images/wallpapers/titan-quest-ii-game-2560x1440-17991.jpg",
  ]);
  assert.deepEqual(parse4kwallpapersDirect(""), []);
});
...
test("fromAlphacoders 站内直抓：search.php 专题页原图直链命中（替代 Bing site:）", async () => {
  ...
  const html =
    '<a href="' +
    full +
    '">x</a><img src="https://images.alphacoders.com/117/thumb-1920-1179971.jpg">';
  const downloaded = [];
  const c = makeCover((url) => {
  ...
  const r = await c.fromAlphacoders("Alan Wake Remastered", "D:\\out", {
    query: "Alan Wake Remastered",
  });
  assert.equal(r.ok, true);
...
test("fromAlphacoders 站内搜索失败 / 无直链时干净失败", async () => {
  ...
  const r1 = await c1.fromAlphacoders("Alan Wake Remastered", "D:\\out", {
    query: "Alan Wake Remastered",
  });
  ...
  const r2 = await c2.fromAlphacoders("Alan Wake Remastered", "D:\\out", {
    query: "Alan Wake Remastered",
  });
...
test("resolveSteamAppId 版本词剥离降级：完整词 0 结果时用基础名再查（对齐 kdocs）", async () => {
  const game = "Mystery Saga Remastered";
  const base = "Mystery Saga";
  const fullUrl =
    "https://store.steampowered.com/api/storesearch/?term=" +
    encodeURIComponent(game) +
    "&l=english&cc=US";
      ...
      if (url === baseUrl) {
        return {
          json: {
            total: 7,
            items: [
              { id: 108710, name: "Mystery Saga" },
              { id: 2661250, name: "Something Mystery Saga Chapter" },
            ],
          },
        };
      }
  ...
  assert.equal(await c.resolveSteamAppId(game), "108710");
  assert.deepEqual(
    calls.map((x) => x.url),
    [fullUrl, baseUrl],
    "先查完整词，0 结果后降级基础名",
...
test("fromReddit 相关性闸门：只采纳标题相关的贴，无关热门贴被剔除", async () => {
  ...
  const redditUrl =
    "https://www.reddit.com/r/gamewallpaper/search.json?q=" +
    encodeURIComponent("Just Cause 4") +
...
test("fetchCover Bing 相关性拦截后继续降级到下一来源，不会把错图当成功", async () => {
  const fs = fakeFs();
  const bingHtml = bingItemHtml({
    murl: "https://cdn.4kwallpapers.com/kagurabachi.jpg",
    purl: "https://4kwallpapers.com/kagura",
    t: "Kagurabachi",
  });
  const wallhavenImg = "https://w.wallhaven.cc/full/ab/wallhaven-jc4.jpg";
  const c = makeCover((url) => {
    if (url.startsWith(BING_IMAGE_SEARCH) && url.includes("4kwallpapers"))
      return { text: bingHtml };
    if (url.includes("wallhaven.cc/api")) {
...
test("isYouTubeTitleRelevant：中文搜索词+数字，宣传片标题且编号匹配 → 采纳", () => {
  ...
  assert.equal(
    isYouTubeTitleRelevant(
      "The Legend of Zelda - Nintendo Switch Presentation 2017 Trailer",
      "塞尔达传说",
    ),
    true,
  );
...
test("applyCandidate：用户选中即落盘，低于 1280×720 如实标注 degraded", async () => {
  ...
  const r = await cover.applyCandidate(
    "https://x.example/cover.jpg",
    "E:\\素材\\【游戏268】正当防卫4",
    { source: "youtube" },
  );
  assert.equal(r.ok, true);
...
test("extractEmbeddedEnglishTitle：混合标题提取最长拉丁片段并剥版本尾巴", () => {
  ...
  assert.equal(
    extractEmbeddedEnglishTitle("只狼：影逝二度 Sekiro: Shadows Die Twice v1.06"),
