...
async function loadTokensFromProfile() {
  ...
  try {
    ...
    await page.goto(ORIGIN + '/', { waitUntil: 'domcontentloaded', timeout: 25000 }).catch(() => {});
    await page.waitForTimeout(3000);
    const t = await page.evaluate(() => {
      let c = {};
      try { c = JSON.parse(localStorage.getItem('credentials_Xqp0kJBXWhwaTpB6') || '{}'); } catch (e) {}
      let cap = '';
      try { cap = (JSON.parse(localStorage.getItem('captcha_Xqp0kJBXWhwaTpB6') || '{}').token) || ''; } catch (e) {}
      return {
  ...
  if (tokenCache.refresh_token && !force) {
    try {
      const r = await fetch(AUTH_BASE + '/v1/auth/token', {
        ...
        body: JSON.stringify({ client_id: CLIENT_ID, grant_type: 'refresh_token', refresh_token: tokenCache.refresh_token }),
      });
...
async function loadTokensFromProfileGuarded() {
  ...
  profileLoadPromise = (async () => {
    try { return await loadTokensFromProfile(); }
...
async function getValidAuth() {
  ...
  return { token: tokenCache.access_token, captcha: tokenCache.captcha, deviceId: tokenCache.device_id };
}
...
async function apiCall(method, url, { query, body, _depth = 0 } = {}) {
  ...
  const isCaptchaInvalid = (resp.status === 400 && j && (j.error === 'captcha_invalid' || j.error_code === 9));
...
async function listFiles(parentId) {
  ...
  do {
    const query = {
      ...
      with_audit: 'true', thumbnail_size: 'SIZE_SMALL', limit: '50',
    };
    if (pageToken) query.page_token = pageToken;
    const r = await apiCall('GET', API_BASE + '/files', { query });
    if (r.status !== 200) throw new Error('列目录失败(' + r.status + '): ' + (r.text || '').slice(0, 200));
    const files = (r.json && r.json.files) || [];
    all = all.concat(files.map((f) => ({ id: f.id, name: f.name, size: f.size, kind: f.kind, type: f.type })));
...
async function getShareInfo(shareId, pwd) {
  const r = await apiCall('GET', API_BASE + '/share', {
    query: { share_id: shareId, pass_code: pwd || '', limit: '100', pass_code_token: '', page_token: '', thumbnail_size: 'SIZE_SMALL' },
  });
  const data = (r.json && (r.json.data || r.json)) || {};
  const status = (r.json && (r.json.share_status || data.share_status));
  if (status && status !== 'OK') throw new Error('分享无效(' + status + '):' + (r.text || '').slice(0, 200));
  const files = data.files || [];
  if (!files.length) throw new Error('该分享为空或无法读取文件列表(可能需重新登录)');
  return { files: files.map((f) => ({ id: f.id, name: f.name, size: f.size })), passCodeToken: data.pass_code_token || '' };
}
...
async function restore(shareId, passCodeToken, fileIds, parentId) {
  const r = await apiCall('POST', API_BASE + '/share/restore', {
    body: {
      parent_id: parentId || '', share_id: shareId, pass_code_token: passCodeToken,
      ancestor_ids: [], specify_parent_id: true, file_ids: fileIds,
    },
  });
  if (r.status !== 200) throw new Error('转存失败(' + r.status + '): ' + (r.text || '').slice(0, 200));
  const data = r.json || {};
...
async function waitTask(taskId) {
  for (let i = 0; i < 40; i++) {
    ...
    if (r.json && r.json.progress === 100) {
      ...
      if (trace) { try { return Object.values(JSON.parse(trace)); } catch (e) {} }
    }
  }
...
async function createShare(fileIds) {
  const r = await apiCall('POST', API_BASE + '/share', {
    body: {
      file_ids: fileIds, share_to: 'copy',
      params: { subscribe_push: 'false', WithPassCodeInLink: 'true' },
      title: '云盘资源分享', restore_limit: '-1', expiration_days: '-1',
    },
  });
  if (r.status !== 200) throw new Error('创建分享失败(' + r.status + '): ' + (r.text || '').slice(0, 200));
  const data = r.json || {};
...
async function transfer({ link, pwd, makeShare, sharePeriod, sharePassword, destFolderId, destFolderName }) {
  ...
  const taskId = await restore(shareId, info.passCodeToken, info.files.map((f) => f.id), game.id);
  const fileList = info.files.map((f) => ({ path: '/' + game.name + '/' + f.name, server_filename: f.name, size: f.size }));

  let newFileIds = [];
  let waitError = null;
  try { newFileIds = await waitTask(taskId); }
  catch (e) { waitError = e.message; } // 轮询失败先记下,转存可能已成功但拿不到新 id

  let share = null;
  let shareError = null;
  if (makeShare) {
    if (!newFileIds.length) {
      shareError = waitError ? ('转存任务未完成,无法生成分享:' + waitError) : '转存任务未完成,无法生成分享';
    } else {
      try { share = await createShare(newFileIds); }
...
async function saveShare(opts) {
  ...
  const taskId = await restore(shareId, info.passCodeToken, info.files.map((f) => f.id), opts.destId || '');
  return { restore_task_id: taskId, task_id: taskId, destPath: '/' + (opts.destName || '') };
}

async function probeListRoot() {
  try { const files = await listFiles(''); return { ok: true, count: files.length, sample: files.slice(0, 5) }; }
